# 🎙️ PAZHVAK: Persian Speech Recognition Dataset  
**First Public Dataset for Persian Word-Level ASR**  

🔗 **Dataset Link**: 
      👉 [PAZHVAK_DATASET](https://hormozgan.ac.ir/home/index/33/91/2164)  

🔗 **Pretrained Model**: [Coming Soon]   


## 🚀 Overview  
Speech recognition is a critical task in voice technology, yet **low-resource languages like Persian lack high-quality datasets**. PAZHVAK is the **first public Persian word-level ASR dataset**, containing:  
- 88,535 audio files  
- 4,018 common Persian words  
- Recorded by 61 students (38 male, 23 female) at University of Hormozgan  


## 🔍 Key Features  
- 📖 **4,018-word vocabulary** classification  
- 🤖 **Open-source** pretrained models  


## 🛠️ Tech Stack  
```python
# Core Tools
Python · PyTorch · Hugging Face Transformers

# Audio Processing
Librosa · TorchAudio · Weights & Biases (for callbacks)

# Deployment
Gradio (for demo)
```


## 🤝 How to Cite
```
@dataset{pazhvak2024,
  title={PAZHVAK Persian ASR Dataset},
  author={University of Hormozgan},
  year={2024}
}
```
